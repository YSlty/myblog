<template>
  <div class="hello">
		...
		<div class="index-list">
			...
			<ul class="list-cont">
				<li v-for="(item, index) in datas.data" v-bind:key="index" :data-id="item.id" @click="enterDetails(item.id)">
					<p class="list-img"><img src="" alt="" title=""></p>
					<div class="cont">
						<h3>{{item.title}}</h3>
						<p v-html="item.jianjie"></p>
						<div>
							<p><span>{{item.type}}</span><span>{{item.time}}</span></p>
		...
		<div class="sidebar">
			...
			<div class="links mt30">
				...
				<ul>
					<li><a href="http://www.zcx1.com/#" target="_blank">星仔个人笔记</a></li>
				</ul>
			</div>
